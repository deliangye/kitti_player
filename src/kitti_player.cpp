...
struct kitti_player_options {
  ...
  bool
      sendTransform; // publish velodyne TF IMU 3DOF orientation wrt fixed frame
  bool stereoDisp;   // use precalculated stereoDisparities
  bool viewDisparities; // view use precalculated stereoDisparities
  bool synchMode; // start with synchMode on (wait for message to send next
                  // frame)
...
int publish_velodyne(ros::Publisher &pub, string infile,
  ...
  if (!input.good()) {
    ...
    points->header.stamp = pcl_conversions::toPCL(time);
...
int publish_velodyne_pieces(ros::Publisher &pub, string infile,
                            const ros::Time &start_time,
                            const ros::Time &end_time,
                            float horiz_angle_degree) {
  //ROS_INFO_STREAM("calling publish_velodyne_pieces");
  fstream input(infile.c_str(), ios::in | ios::binary);
  if (!input.good()) {
    ROS_ERROR_STREAM("Could not read file: " << infile);
    return 0;
    ...
    if (horiz_angle_degree <= 0.0f || horiz_angle_degree >= 360.0) {
      ROS_ERROR_STREAM("invalid horiz_angle_degree " << horiz_angle_degree);
      return 0;
    }

    int ngroups = std::floor(360.0 / horiz_angle_degree);
    std::vector<pcl::PointCloud<pcl::PointXYZI>> grouped_points(ngroups);
    ...
    for (int i = 0; input.good() && !input.eof(); i++) {
      ...
      float deg = (std::atan2(point.y, point.x) / M_PI * 180.0) + 180.0;
      int group_id = std::floor(deg / horiz_angle_degree);
      if (group_id < 0) {
        group_id = 0;
      }
      if (group_id >= ngroups) {
      ...
      grouped_points[group_id].push_back(point);
    }
    input.close();
    ...
    for (int i = 0; i < ngroups; i++) {
      ros::Time current_timestamp =
          start_time + (end_time - start_time) * (1.0f / float(ngroups));
      auto &points = grouped_points[i];
      points.header.frame_id = "base_link";
      points.header.stamp = pcl_conversions::toPCL(current_timestamp);

      // workaround for the PCL headers...
      // http://wiki.ros.org/hydro/Migration#PCL
      sensor_msgs::PointCloud2 pc2;
      pcl::toROSMsg(points, pc2);
      //ROS_INFO_STREAM("published " << i << "-th velodyne piece");
      pub.publish(pc2);
    }
 ...
 * @param P double P[12] - Projection Matrix Rectified (u,v,w) = P * R *
 * (x,y,z,q)
 ...
 */
int getCalibration(string dir_root, string camera_name, double *K,
                   std::vector<double> &D, double *R, double *P) {
  ...
  ROS_INFO_STREAM("Reading camera" << camera_name << " calibration from "
                                   << calib_cam_to_cam);
  ...
  while (getline(file_c2c, line)) {
    ...
    // Move the iterator at the beginning of the tokenize vector and check for
    // K/D/R/P matrices.
    token_iterator = tok.begin();
    if (strcmp((*token_iterator).c_str(),
               ((string)(string("K_") + camera_name + string(":"))).c_str()) ==
        0) // Calibration Matrix
    {
    ...
    //        token_iterator=tok.begin();
    //        if
    //        (strcmp((*token_iterator).c_str(),((string)(string("D_")+camera_name+string(":"))).c_str())==0)
    //        //Distortion Coefficients
    //        {
    //            index=0; //should be 5 at the end
    //            ROS_DEBUG_STREAM("D_" << camera_name);
    //            for (token_iterator++; token_iterator != tok.end();
    //            token_iterator++)
    //            {
    ////                std::cout << *token_iterator << '\n';
    ...
    token_iterator = tok.begin();
    if (strcmp((*token_iterator).c_str(),
               ((string)(string("R_") + camera_name + string(":"))).c_str()) ==
        0) // Rectification Matrix
    {
    ...
    token_iterator = tok.begin();
    if (strcmp((*token_iterator).c_str(),
               ((string)(string("P_rect_") + camera_name + string(":")))
                   .c_str()) == 0) // Projection Matrix Rectified
    {
      index = 0; // should be 12 at the end
...
int getGPS(string filename, sensor_msgs::NavSatFix *ros_msgGpsFix,
  ...
  ros_msgGpsFix->position_covariance_type =
...
Xy latlon2xy_helper(double lat, double lngd) {
  ...
  M = M -
      sin(2.0 * phi) *
          (esq * (3.0 / 8.0 + esq * (3.0 / 32.0 + 45.0 * esq / 1024.0)));
  ...
  double x =
      ...
      (1.0 +
       A * A *
           ((1.0 - T + C) / 6.0 +
            A * A * (5.0 - 18.0 * T + T * T + 72.0 * C - 58.0 * e0sq) / 120.0));
 ...
 * Allowed options:
 *   -h [ --help ]                       help message
 *   -d [ --directory  ] arg             *required* - path to the kitti dataset
 * Directory
...
int main(int argc, char **argv) {
  ...
  po::options_description desc("Kitti_player, a player for KITTI raw "
                               "datasets\nDatasets can be downloaded from: "
                               "http://www.cvlibs.net/datasets/kitti/"
                               "raw_data.php\n\nAllowed options",
                               200);
  desc.add_options()("help,h", "help message")(
      "directory ,d", po::value<string>(&options.path)->required(),
      "*required* - path to the kitti dataset Directory")(
      "frequency ,f", po::value<float>(&options.frequency)->default_value(1.0),
      "set replay Frequency")(
      "all       ,a",
      po::value<bool>(&options.all_data)->default_value(0)->implicit_value(1),
      "replay All data")(
      "velodyne  ,v",
      po::value<bool>(&options.velodyne)->default_value(0)->implicit_value(1),
      "replay Velodyne data")("veloPieceAngle,P",
                              po::value<float>(&options.velodyne_piece_angle)
                                  ->default_value(0.0)
                                  ->implicit_value(4.5),
                              "replay piecewise velodyne angle")(
      "gps       ,g",
      po::value<bool>(&options.gps)->default_value(0)->implicit_value(1),
      "replay Gps data")(
      "imu       ,i",
      po::value<bool>(&options.imu)->default_value(0)->implicit_value(1),
      "replay Imu data")(
      "grayscale ,G",
      po::value<bool>(&options.grayscale)->default_value(0)->implicit_value(1),
      "replay Stereo Grayscale images")(
      "color     ,C",
      po::value<bool>(&options.color)->default_value(0)->implicit_value(1),
      "replay Stereo Color images")(
      "viewer    ,V",
      po::value<bool>(&options.viewer)->default_value(0)->implicit_value(1),
      "enable image viewer")(
      "timestamps,T",
      po::value<bool>(&options.timestamps)->default_value(0)->implicit_value(1),
      "use KITTI timestamps")(
      "stereoDisp,s",
      po::value<bool>(&options.stereoDisp)->default_value(0)->implicit_value(1),
      "use pre-calculated disparities")(
      "viewDisp  ,D ", po::value<bool>(&options.viewDisparities)
                           ->default_value(0)
                           ->implicit_value(1),
      "view loaded disparity images")(
      "frame     ,F", po::value<unsigned int>(&options.startFrame)
                          ->default_value(0)
                          ->implicit_value(0),
      "start playing at frame...")(
      "gpsPoints ,p",
      po::value<string>(&options.gpsReferenceFrame)->default_value(""),
      "publish GPS/RTK markers to RVIZ, having reference frame as "
      "<reference_frame> [example: -p map]")(
      "synchMode ,S",
      po::value<bool>(&options.synchMode)->default_value(0)->implicit_value(1),
      "Enable Synch mode (wait for signal to load next frame [std_msgs/Bool "
      "data: true]");

  try // parse options
  {
    po::parsed_options parsed = po::command_line_parser(argc - 2, argv)
                                    .options(desc)
                                    .allow_unregistered()
                                    .run();
    po::store(parsed, vm);
    ...
    //        {
    //            ROS_WARN_STREAM("Unknown Options Detected, shutting down
    //            node\n");
    //            cerr << desc << endl;
    //            return 1;
    //        }
  ...
  image_transport::CameraPublisher pub00 =
      it.advertiseCamera("grayscale/left/image_rect", 1);
  image_transport::CameraPublisher pub01 =
      it.advertiseCamera("grayscale/right/image_rect", 1);
  image_transport::CameraPublisher pub02 =
      it.advertiseCamera("color/left/image_rect", 1);
  image_transport::CameraPublisher pub03 =
      it.advertiseCamera("color/right/image_rect", 1);
  ...
  //     node.advertise<pcl::PointCloud<pcl::PointXYZ>>("hdl64e", 1, true);
  ros::Publisher lidar_pub =
      node.advertise<sensor_msgs::PointCloud2>("lidar", 10, true);
  ros::Publisher lidar_piece_pub =
      node.advertise<sensor_msgs::PointCloud2>("lidar_piece", 10, true);

  ros::Publisher gps_pub =
      node.advertise<sensor_msgs::NavSatFix>("oxts/gps", 1, true);
  ros::Publisher gps_pub_initial =
      node.advertise<sensor_msgs::NavSatFix>("oxts/gps_initial", 1, true);
  ros::Publisher imu_pub =
      node.advertise<sensor_msgs::Imu>("oxts/imu", 1, true);
  ros::Publisher disp_pub = node.advertise<stereo_msgs::DisparityImage>(
      "preprocessed_disparity", 1, true);

  sensor_msgs::NavSatFix ros_msgGpsFix;
  sensor_msgs::NavSatFix ros_msgGpsFixInitial; // This message contains the
                                               // first reading of the file
  bool firstGpsData = true; // Flag to store the ros_msgGpsFixInitial message
  sensor_msgs::Imu ros_msgImu;

  ros::Subscriber sub =
      node.subscribe("/kitti_player/synch", 1, synchCallback); // refs #600
  ...
  if (!(options.all_data || options.color || options.gps || options.grayscale ||
        options.imu || options.velodyne)) {
    ROS_WARN_STREAM("Job finished without playing the dataset. No 'publishing' "
                    "parameters provided");
    // node.shutdown();
    return 1;
  }
  ...
  (*(options.path.end() - 1) != '/' ? dir_root = options.path + "/"
                                    : dir_root = options.path);
  (*(options.path.end() - 1) != '/'
       ? dir_image00 = options.path + "/image_00/data/"
       : dir_image00 = options.path + "image_00/data/");
  (*(options.path.end() - 1) != '/'
       ? dir_image01 = options.path + "/image_01/data/"
       : dir_image01 = options.path + "image_01/data/");
  (*(options.path.end() - 1) != '/'
       ? dir_image02 = options.path + "/image_02/data/"
       : dir_image02 = options.path + "image_02/data/");
  (*(options.path.end() - 1) != '/'
       ? dir_image03 = options.path + "/image_03/data/"
       : dir_image03 = options.path + "image_03/data/");
  (*(options.path.end() - 1) != '/'
       ? dir_image04 = options.path + "/disparities/"
       : dir_image04 = options.path + "disparities/");
  (*(options.path.end() - 1) != '/' ? dir_oxts = options.path + "/oxts/data/"
                                    : dir_oxts = options.path + "oxts/data/");
  (*(options.path.end() - 1) != '/'
       ? dir_velodyne_points = options.path + "/velodyne_points/data/"
       : dir_velodyne_points = options.path + "velodyne_points/data/");

  (*(options.path.end() - 1) != '/'
       ? dir_timestamp_image00 = options.path + "/image_00/"
       : dir_timestamp_image00 = options.path + "image_00/");
  (*(options.path.end() - 1) != '/'
       ? dir_timestamp_image01 = options.path + "/image_01/"
       : dir_timestamp_image01 = options.path + "image_01/");
  (*(options.path.end() - 1) != '/'
       ? dir_timestamp_image02 = options.path + "/image_02/"
       : dir_timestamp_image02 = options.path + "image_02/");
  (*(options.path.end() - 1) != '/'
       ? dir_timestamp_image03 = options.path + "/image_03/"
       : dir_timestamp_image03 = options.path + "image_03/");
  (*(options.path.end() - 1) != '/'
       ? dir_timestamp_oxts = options.path + "/oxts/"
       : dir_timestamp_oxts = options.path + "oxts/");
  (*(options.path.end() - 1) != '/'
       ? dir_timestamp_velodyne = options.path + "/velodyne_points/"
       : dir_timestamp_velodyne = options.path + "velodyne_points/");
  ...
  if ((options.all_data && ((opendir(dir_image00.c_str()) == NULL) ||
  ...
  if (options.viewer) {
    ...
    if (options.color || options.all_data) {
      ROS_DEBUG_STREAM("color||all " << options.color << " "
                                     << options.all_data);
      cv::namedWindow("CameraSimulator Color Viewer", CV_WINDOW_AUTOSIZE);
      full_filename_image02 =
          dir_image02 + boost::str(boost::format("%010d") % 0) + ".png";
      cv_image02 = cv::imread(full_filename_image02, CV_LOAD_IMAGE_UNCHANGED);
      cv::waitKey(5);
    }
    if (options.grayscale || options.all_data) {
      ROS_DEBUG_STREAM("grayscale||all " << options.grayscale << " "
                                         << options.all_data);
      cv::namedWindow("CameraSimulator Grayscale Viewer", CV_WINDOW_AUTOSIZE);
      full_filename_image00 =
          dir_image00 + boost::str(boost::format("%010d") % 0) + ".png";
      cv_image00 = cv::imread(full_filename_image00, CV_LOAD_IMAGE_UNCHANGED);
      cv::waitKey(5);
    }
    if (options.viewDisparities || options.all_data) {
      ROS_DEBUG_STREAM("viewDisparities||all " << options.grayscale << " "
                                               << options.all_data);
      cv::namedWindow("Precomputed Disparities", CV_WINDOW_AUTOSIZE);
      full_filename_Disparities =
          dir_Disparities + boost::str(boost::format("%010d") % 0) + ".png";
      cv_disparities =
          cv::imread(full_filename_Disparities, CV_LOAD_IMAGE_UNCHANGED);
      cv::waitKey(5);
    }
  ...
  if (options.color || options.all_data) {
    if (!(getCalibration(dir_root, "02", ros_cameraInfoMsg_camera02.K.data(),
                         ros_cameraInfoMsg_camera02.D,
                         ros_cameraInfoMsg_camera02.R.data(),
                         ros_cameraInfoMsg_camera02.P.data()) &&
          getCalibration(dir_root, "03", ros_cameraInfoMsg_camera03.K.data(),
                         ros_cameraInfoMsg_camera03.D,
                         ros_cameraInfoMsg_camera03.R.data(),
                         ros_cameraInfoMsg_camera03.P.data()))) {
    ...
    cv_image02 = cv::imread(full_filename_image02, CV_LOAD_IMAGE_UNCHANGED);
    cv::waitKey(5);
    ros_cameraInfoMsg_camera03.height = ros_cameraInfoMsg_camera02.height =
        cv_image02.rows; // -1;TODO: CHECK, qui potrebbe essere -1
    ros_cameraInfoMsg_camera03.width = ros_cameraInfoMsg_camera02.width =
        cv_image02.cols; // -1;
  }

  if (options.grayscale || options.all_data) {
    if (!(getCalibration(dir_root, "00", ros_cameraInfoMsg_camera00.K.data(),
                         ros_cameraInfoMsg_camera00.D,
                         ros_cameraInfoMsg_camera00.R.data(),
                         ros_cameraInfoMsg_camera00.P.data()) &&
          getCalibration(dir_root, "01", ros_cameraInfoMsg_camera01.K.data(),
                         ros_cameraInfoMsg_camera01.D,
                         ros_cameraInfoMsg_camera01.R.data(),
                         ros_cameraInfoMsg_camera01.P.data()))) {
    ...
    cv_image00 = cv::imread(full_filename_image00, CV_LOAD_IMAGE_UNCHANGED);
    cv::waitKey(5);
    ros_cameraInfoMsg_camera01.height = ros_cameraInfoMsg_camera00.height =
        cv_image00.rows; // -1; TODO: CHECK -1?
    ros_cameraInfoMsg_camera01.width = ros_cameraInfoMsg_camera00.width =
        cv_image00.cols; // -1;
  }
  ...
  publisher_GT_RTK = node.advertise<visualization_msgs::MarkerArray>(
      "/kitti_player/GT_RTK", 1);
  ...
  do {
    ...
    if (options.stereoDisp) {
      ...
      full_filename_image04 =
          dir_image04 + boost::str(boost::format("%010d") % entries_played) +
          ".png";
    ...
    if (options.viewDisparities) {
      full_filename_Disparities =
          dir_Disparities +
          boost::str(boost::format("%010d") % entries_played) + ".png";
      cv_disparities =
          cv::imread(full_filename_Disparities, CV_LOAD_IMAGE_UNCHANGED);
      cv::putText(cv_disparities, "KittiPlayer", cvPoint(20, 15),
                  CV_FONT_HERSHEY_SIMPLEX, 0.4, cvScalar(0, 255, 0), 1, CV_AA);
      cv::putText(cv_disparities,
                  boost::str(boost::format("%5d") % entries_played),
                  cvPoint(cv_disparities.size().width - 100, 30),
                  CV_FONT_HERSHEY_DUPLEX, 1.0, cvScalar(0, 0, 255), 1, CV_AA);
      cv::waitKey(5);
    }

    if (options.color || options.all_data) {
      full_filename_image02 =
          dir_image02 + boost::str(boost::format("%010d") % entries_played) +
          ".png";
      full_filename_image03 =
          dir_image03 + boost::str(boost::format("%010d") % entries_played) +
          ".png";
      ROS_DEBUG_STREAM(full_filename_image02 << endl
                                             << full_filename_image03 << endl
                                             << endl);
      ...
      if (!options.timestamps) {
        ...
        ros_msg02.header.stamp = ros_cameraInfoMsg_camera02.header.stamp =
            cv_bridge_img.header.stamp;
      } else {
      ...
      if (!options.timestamps) {
        cv_bridge_img.header.stamp = current_timestamp;
        ros_msg03.header.stamp = ros_cameraInfoMsg_camera03.header.stamp =
            cv_bridge_img.header.stamp;
      } else {
    ...
    if (options.grayscale || options.all_data) {
      full_filename_image00 =
          dir_image00 + boost::str(boost::format("%010d") % entries_played) +
          ".png";
      full_filename_image01 =
          dir_image01 + boost::str(boost::format("%010d") % entries_played) +
          ".png";
      ROS_DEBUG_STREAM(full_filename_image00 << endl
                                             << full_filename_image01 << endl
                                             << endl);
      ...
      if ((cv_image00.data == NULL) || (cv_image01.data == NULL)) {
        ...
        ROS_ERROR_STREAM(full_filename_image00 << endl
                                               << full_filename_image01);
        // node.shutdown();
        return -1;
      }
      ...
      if (!options.timestamps) {
        cv_bridge_img.header.stamp = current_timestamp;
        ros_msg00.header.stamp = ros_cameraInfoMsg_camera00.header.stamp =
            cv_bridge_img.header.stamp;
      } else {
      ...
      if (!options.timestamps) {
        cv_bridge_img.header.stamp = current_timestamp;
        ros_msg01.header.stamp = ros_cameraInfoMsg_camera01.header.stamp =
            cv_bridge_img.header.stamp;
      } else {
    ...
    if (options.velodyne || options.all_data) {
      ...
      full_filename_velodyne =
          dir_velodyne_points +
          boost::str(boost::format("%010d") % entries_played) + ".bin";

      Time next_timestamp =
          current_timestamp + Duration(1.0 / options.frequency);
      ...
      if (options.timestamps) {
        ...
        if (timestamps) {
          getline(timestamps, str_support);
          next_timestamp = parseTime(str_support).stamp;
        } else {
          next_timestamp = header_support.stamp + ros::Duration(0.1);
        }
      }

      //ROS_INFO_STREAM("options.velodyne_piece_angle = " << options.velodyne_piece_angle);

      publish_velodyne(lidar_pub, full_filename_velodyne, header_support.stamp);
      if (options.velodyne_piece_angle > 0.0) {
        publish_velodyne_pieces(lidar_piece_pub, full_filename_velodyne,
                                header_support.stamp, next_timestamp,
                                options.velodyne_piece_angle);
      }
    }
    ...
    if (options.gps || options.all_data) {
      ...
      full_filename_oxts = dir_oxts +
                           boost::str(boost::format("%010d") % entries_played) +
                           ".txt";
      ...
      if (options.gpsReferenceFrame.length() > 1) {
        ...
        Xy xyFromLatLon;
        xyFromLatLon =
            latlon2xy_helper(ros_msgGpsFix.latitude, ros_msgGpsFix.longitude);
        ...
        ROS_DEBUG_STREAM(RTK_MARKER.pose.position.x
                         << "\t" << RTK_MARKER.pose.position.y);
